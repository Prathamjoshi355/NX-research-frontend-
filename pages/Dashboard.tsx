...
const Dashboard = () => {
  ...
  const renderContent = () => {
    switch (activeView) {
      case 'applications':
        return (
          <div className="animate-in fade-in duration-300">
            <h2 className="text-2xl font-black text-[#0A2463] mb-8 uppercase tracking-tighter">My Applications</h2>
                ...
                <tbody className="divide-y divide-gray-50">
                  {applications.map((app, i) => (
                    <tr key={i} className="hover:bg-gray-50/30">
                      <td className="px-8 py-6 font-bold text-[#0A2463] text-sm uppercase">{app.domain}</td>
                      ...
                      <td className="px-8 py-6 text-right">
                        <button className="text-[#FB8500] font-bold text-[10px] uppercase tracking-widest hover:underline">View</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      case 'apply':
        return (
          <div className="animate-in fade-in duration-300">
            <h2 className="text-2xl font-black text-[#0A2463] mb-8 uppercase tracking-tighter">Apply New</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {['Summit', 'Gov Research', 'Private Research', 'Student Initiative', 'Empowerment'].map(dom => (
                <Card key={dom} className="p-8 bg-white text-center">
                  <h3 className="font-black text-[#0A2463] uppercase text-sm mb-6">{dom}</h3>
      ...
      case 'profile':
        return (
          <div className="animate-in fade-in duration-300 max-w-2xl">
            <h2 className="text-2xl font-black text-[#0A2463] mb-8 uppercase tracking-tighter">My Profile</h2>
        ...
        return (
          <div className="animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-12">
              {['Summit', 'Gov Research', 'Private Research', 'Initiative', 'Empowerment'].map(dom => (
                <Card key={dom} className="p-6 bg-white border-none shadow-sm flex flex-col items-center hover:scale-105 transition-transform cursor-pointer" onClick={() => setActiveView('apply')}>
                  ...
                  <div className="bg-[#0A2463] text-white p-2 rounded-lg"><PlusCircle size={16} /></div>
                </Card>
              ))}
            </div>
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setActiveView('applications')} className="text-[10px] font-black uppercase text-[#FB8500] tracking-widest hover:underline">View All</button>
            </div>
            <Card className="p-0 border-none shadow-xl rounded-[40px] overflow-hidden bg-white">
              <table className="w-full text-left">
                <thead className="bg-gray-50/50 text-[10px] font-black uppercase tracking-widest text-gray-400">
  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FA]">
      {/* SIDEBAR */}
      <aside className="w-64 bg-[#0A2463] text-white p-8 flex flex-col fixed h-full z-10">
        ...
        <nav className="space-y-2 flex-grow">
          {navItems.map(item => (
            <button 
              ...
              className={`flex items-center space-x-3 w-full p-4 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeView === item.id ? 'bg-white text-[#0A2463] shadow-lg' : 'hover:bg-white/5 text-white/60'}`}
            >
      ...
      <main className="ml-64 flex-grow p-12">
        <header className="mb-12 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-black text-[#0A2463] uppercase tracking-tighter">
          ...
          <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-gray-100 flex items-center space-x-3">
            <div className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-[#0A2463]"><User size={16} /></div>
            <span className="font-black text-[#0A2463] text-xs uppercase tracking-widest">ID: 8921</span>
          </div>
        </header>
