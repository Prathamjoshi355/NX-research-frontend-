...
const StudentInitiative = () => (
  <div className="animate-in slide-in-from-left duration-500">
    ...
    <Section className="py-24 max-w-4xl mx-auto">
      <h2 className="text-3xl font-black text-[#0A2463] mb-8 uppercase tracking-tighter">What is Student Initiative</h2>
      <p className="text-xl text-gray-600 leading-relaxed font-medium">NX Research is a student-initiative driven organization where students take ownership of problem identification, solution design and implementation, while mentors provide direction and review.</p>
    </Section>

    <Section gray>
      <h2 className="text-3xl font-black text-[#0A2463] mb-12 uppercase tracking-tighter text-center">How Students Work</h2>
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        {['Choose a problem', 'Propose solution', 'Design system', 'Build & test', 'Document & present'].map((step, i) => (
          <div key={i} className="text-center bg-white p-6 rounded-2xl shadow-sm"><div className="font-black text-[#FB8500] text-2xl mb-2">{i+1}</div><h4 className="font-bold text-xs uppercase tracking-tight text-[#0A2463]">{step}</h4></div>
        ))}
      </div>
    </Section>
