  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FA]">
      <aside className="w-64 bg-[#1E1E1E] text-white p-8 flex flex-col fixed h-full z-10">
        ...
        <nav className="space-y-2 flex-grow">
          {[
            ...
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center space-x-3 w-full p-4 rounded-xl font-bold text-[10px] uppercase tracking-widest ${activeTab === item.id ? 'bg-[#FB8500]' : 'hover:bg-white/5'}`}>
      ...
      <main className="ml-64 flex-grow p-16">
        <h1 className="text-3xl font-black text-[#0A2463] uppercase tracking-tighter mb-12">Industry Partner Panel</h1>
        <div className="grid grid-cols-1 gap-8">
          <Card className="p-12 bg-white border-none shadow-sm rounded-[3rem]">
            <h3 className="text-xl font-black text-[#0A2463] mb-8 uppercase">Active Research Roadmap</h3>
            <div className="relative border-l-2 border-gray-100 ml-4 space-y-12">
              {[
                ...
                <div key={i} className="relative pl-10">
                  <div className={`absolute -left-[11px] w-5 h-5 rounded-full border-4 border-white ${s.status === 'Completed' ? 'bg-[#06A77D]' : 'bg-gray-200'}`}></div>
