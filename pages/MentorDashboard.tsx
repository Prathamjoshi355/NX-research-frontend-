  ...
  return (
    <div className="flex min-h-screen bg-[#F8F9FA]">
      <aside className="w-64 bg-[#0A2463] text-white p-8 flex flex-col fixed h-full z-10">
        ...
        <nav className="space-y-2 flex-grow">
          {[
            ...
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex items-center space-x-3 w-full p-4 rounded-xl font-bold text-xs uppercase tracking-widest ${activeTab === item.id ? 'bg-[#FB8500]' : 'hover:bg-white/5'}`}>
      ...
      <main className="ml-64 flex-grow p-16">
        <h1 className="text-3xl font-black text-[#0A2463] uppercase tracking-tighter mb-12">Mentor Dashboard</h1>
