...
const SectionHeader = ({ badge, title, subtitle }: { badge: string; title: string; subtitle: string }) => (
  <div className="mb-12 md:mb-20 text-center">
    <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-[#FB8500]/10 border border-[#FB8500]/20 text-[#FB8500] text-[10px] font-black uppercase tracking-[0.3em] mb-6">
      {badge}
    </div>
    <h2 className="text-4xl md:text-7xl font-black text-[#0A2463] uppercase tracking-tighter leading-none italic mb-6">
      {title}
    </h2>
...
const CornerBrackets = () => (
  <>
    <div className="absolute top-8 left-8 w-12 h-12 border-t-2 border-l-2 border-[#FB8500]/20 rounded-tl-2xl"></div>
    <div className="absolute bottom-8 right-8 w-12 h-12 border-b-2 border-r-2 border-[#FB8500]/20 rounded-br-2xl"></div>
  </>
);
    ...
    <div className="animate-in fade-in duration-700 bg-white">
      ...
      <section className="relative h-[92vh] flex items-center justify-center overflow-hidden bg-black">
        ...
        <div className="relative z-10 text-center max-w-6xl px-6">
          <div className="mb-6 md:mb-12 inline-flex items-center px-6 py-2 rounded-full bg-[#FB8500]/20 border border-[#FB8500]/40 text-[#FB8500] text-[10px] md:text-xs font-black uppercase tracking-[0.4em] backdrop-blur-md">
      ...
      <Section className="py-24 md:py-32" ref={sectionRef}>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          <div className="space-y-10">
            <div>
              <h2 className="text-4xl md:text-7xl font-black text-[#0A2463] uppercase tracking-tighter leading-none italic mb-8">
                What is <span className="text-[#FB8500]">FCC</span>?
              </h2>
            ...
            <div className="space-y-6">
              <div className="flex items-start space-x-6">
                <div className="bg-[#FB8500]/10 p-3 rounded-2xl text-[#FB8500] shrink-0"><CheckCircle size={24} /></div>
                <p className="text-lg text-gray-500 leading-relaxed"><span className="font-bold text-[#0A2463]">Collaboration First:</span> We remove the barriers between aspiring researchers and industry giants.</p>
              </div>
              <div className="flex items-start space-x-6">
                <div className="bg-[#FB8500]/10 p-3 rounded-2xl text-[#FB8500] shrink-0"><Layers size={24} /></div>
                <p className="text-lg text-gray-500 leading-relaxed"><span className="font-bold text-[#0A2463]">Zero Hierarchy:</span> Every idea is judged on merit, whether it comes from a first-year student or a veteran founder.</p>
              </div>
            </div>

            <Button size="lg" variant="outline" className="rounded-2xl border-[#0A2463]/10 h-16">Download Brochure</Button>
          </div>

          <div className="relative group h-full flex items-center">
            <div className={`${isPip ? 'fixed bottom-8 right-8 w-64 md:w-80 h-36 md:h-48 z-50 shadow-3xl animate-in slide-in-from-bottom-10 rounded-2xl overflow-hidden border-2 border-[#FB8500]/20 bg-black' : 'relative w-full aspect-video rounded-[3rem] overflow-hidden shadow-2xl transition-all duration-500'}`}>
              <video 
                ref={videoRef}
                autoPlay 
                muted 
              ...
              <button 
                onClick={toggleMute}
                className="absolute bottom-4 right-4 p-2 bg-black/50 text-white rounded-full backdrop-blur-md hover:bg-[#FB8500] transition-colors"
              >
            ...
            {!isPip && (
              <div className="absolute inset-0 bg-[#0A2463]/5 rounded-[3rem] -z-10 translate-x-6 translate-y-6"></div>
            )}
          </div>
        </div>
      </Section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col gap-12 items-center">
            ...
            <div className="w-full flex overflow-x-auto pb-6 gap-6 no-scrollbar justify-start md:justify-center px-2">
              {summitHistory.map(summit => (
                <button 
                  ...
                  className={`flex-shrink-0 px-8 py-6 rounded-[2rem] border transition-all text-left flex items-center space-x-4 ${activeSummit === summit.id ? 'bg-white border-[#FB8500]/50 shadow-xl scale-105' : 'bg-transparent border-black/5 hover:bg-white/40'}`}
                >
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${activeSummit === summit.id ? 'bg-[#0A2463] text-white rotate-6 shadow-md' : 'bg-white/50 text-slate-400'}`}>
                    <Calendar size={22} />
                  </div>
                  <div>
                    <span className={`block text-xl font-black uppercase italic tracking-tighter ${activeSummit === summit.id ? 'text-[#0A2463]' : 'text-slate-400'}`}>
                      {summit.title}
                    </span>
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{summit.city}</span>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="w-full max-w-xl bg-white rounded-[4rem] p-10 md:p-16 border border-black/5 relative overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500 aspect-[3/4] md:aspect-[4/5] flex flex-col">
              ...
              <div className="space-y-10 relative z-10 flex flex-col h-full">
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-5xl font-black text-[#0A2463] tracking-tighter uppercase leading-none italic">{activeSummitData?.title}</h3>
                    {activeSummitData?.status === 'Upcoming' && (
                       <span className="bg-[#FB8500] text-white text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest animate-pulse">Waitlist</span>
                    )}
                  </div>
                  <div className="flex items-center space-x-3 text-gray-400 font-black uppercase tracking-widest text-[11px]">
                    <MapPin size={14} className="text-[#FB8500]" />
                    <span>{activeSummitData?.city}</span>
                    <span>•</span>
                    <span>{activeSummitData?.date}</span>
                  </div>
                  <p className="text-gray-500 text-lg font-medium italic leading-relaxed pt-2">{activeSummitData?.tagline}</p>
                </div>

                <div className="flex-grow space-y-6">
                  {activeSummitData?.details.map((item, i) => (
                    <div key={i} className="bg-gray-50/80 backdrop-blur-sm border border-black/5 p-6 rounded-3xl group hover:border-[#FB8500]/30 transition-all">
                      <h4 className="text-[9px] font-black text-[#FB8500] uppercase tracking-[0.3em] mb-1">{item.label}</h4>
                      <p className="text-base font-black text-[#0A2463] uppercase tracking-tight leading-snug">{item.val}</p>
                    </div>
                  ))}
                </div>
              ...
              {/* Background accent */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-[#FB8500]/5 rounded-full blur-3xl -translate-y-10 translate-x-10"></div>
            </div>
          </div>
        </div>
      </Section>
