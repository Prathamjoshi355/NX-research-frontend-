...
export default function InitiativesHero() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative min-h-[70dvh] sm:min-h-[100dvh] w-full overflow-hidden flex items-center justify-center bg-bg-primary pt-24 pb-12 font-sans"
    >
      {/* ── Video Container ── */}
      <motion.div
        style={
        ...
        className={
          isPip
            ? // PiP mode — fixed bottom-right, big enough to see
              "fixed bottom-8 right-8 z-50 w-[280px] h-[130px] rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.7)] border border-white/10 bg-black"
      ...
      >
        ...
        >
          <source
            src="https://res.cloudinary.com/dhy9pmo8s/video/upload/v1771958595/Initiative_lbvlpa.mp4"
            type="video/mp4"
          />
        </video>
          ...
          {isMuted && (
            <>
              <motion.h1
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
                className="text-3xl xs:text-4xl sm:text-4xl md:text-[64px] font-bold text-text-primary tracking-tighter mb-4 leading-tight drop-shadow-[0_0_60px_rgba(0,212,255,0.3)] uppercase"
              >
                Where Ideas
              </motion.h1>
              ...
              <motion.p
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -20 }}
                className="text-lg xs:text-xl sm:text-xl md:text-3xl font-normal text-neon-cyan mb-10 tracking-[0.2em] sm:tracking-widest drop-shadow-[0_0_30px_rgba(0,212,255,0.6)] px-4"
              >
                Become Impact
              </motion.p>
            </>
