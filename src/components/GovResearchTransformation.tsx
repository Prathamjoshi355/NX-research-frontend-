    ...
    <section className="py-32 bg-bg-secondary relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-24">
          ...
          <p className="mt-4 text-text-secondary font-mono text-xs tracking-[4px] uppercase">The Transformation Flow</p>
        </div>
          ...
          {steps.map((step, i) => (
            <div key={step.title} className="flex flex-col lg:flex-row items-center gap-8 w-full lg:w-auto">
              <motion.div
                ...
                className="w-full lg:w-56 glass-panel p-6 rounded-xl border-neon-cyan/10 text-center relative group"
              >
                <div className="w-8 h-8 rounded-full bg-neon-cyan/20 text-neon-cyan font-mono text-xs font-bold flex items-center justify-center mx-auto mb-4">
                  {i + 1}
                </div>
                <h4 className="text-sm font-display font-bold text-text-primary mb-2">{step.title}</h4>
